...
@login_required(login_url='account_login')
def commentCreateItem(request, slug):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            comment.item = item
            comment.save()
            return redirect('{}#comment_{}'.format(resolve_url('core:product', slug=comment.item.slug), comment.id))
        else:
            return render(request, 'pybo/comment_form.html', {'form': form})


@login_required(login_url='account_login')
def commentModifyItem(request, commentId):
    comment = get_object_or_404(Comment, pk=commentId)
    if request.user != comment.author:
        messages.warning(request, "Not authorized to modify")
        return redirect('core:product', slug=comment.item.slug)
    else:
        if request.method == "GET":
            form = CommentForm(instance=comment)
            context = {'form': form}
...
@login_required(login_url='account_login')
def commentDeleteItem(request, commentId):
    comment = get_object_or_404(Comment, pk=commentId)
    if request.user != comment.author:
        messages.warning(request, 'Not authorized to delete')
        return redirect('core:product', slug=comment.item.slug)
